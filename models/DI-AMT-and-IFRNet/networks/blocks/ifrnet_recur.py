...
class InitDecoder(nn.Module):
    ...
    def forward(self, f0, f1, f_ref, embt, embt_ref, use_sdi=False):
        ...
        # print(f0.shape, f1.shape, embt.shape)
        out = self.convblock(torch.cat([f0, f1, f_ref, embt, embt_ref], 1))
...
class IntermediateDecoder(nn.Module):
    def __init__(self, in_ch, out_ch, skip_ch) -> None:
        super().__init__()
        self.convblock = nn.Sequential(
            convrelu(in_ch * 4 + 4, in_ch * 3),
            ResBlock(in_ch * 3, skip_ch),
            nn.ConvTranspose2d(in_ch * 3, out_ch + 4, 4, 2, 1, bias=True)
        )

    def forward(self, ft_, f0, f1, f_ref, flow0_in, flow1_in):
        f0_warp = warp(f0, flow0_in)
        f1_warp = warp(f1, flow1_in)
        f_in = torch.cat([ft_, f0_warp, f1_warp, f_ref, flow0_in, flow1_in], 1)
