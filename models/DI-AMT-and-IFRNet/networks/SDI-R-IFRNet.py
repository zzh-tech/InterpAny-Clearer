...
class Decoder4(nn.Module):
    ...
    def forward(self, f0, f1, f_ref, embt, embt_ref, use_sdi=False):
        b, c, h, w = f0.shape
        if not use_sdi:
            embt = embt.repeat(1, 1, h, w)
            embt_ref = embt_ref.repeat(1, 1, h, w)
        f_in = torch.cat([f0, f1, f_ref, embt, embt_ref], 1)
...
class Model(nn.Module):
    ...
    def forward(self, img0, img1, img_ref, embt, embt_ref, scale_factor=1.0, eval=False, **kwargs):
        ...
        out4 = self.decoder4(f0_4, f1_4, f_ref_4, embt_4, embt_ref_4, use_sdi=True)
        up_flow0_4 = out4[:, 0:2]
        up_flow1_4 = out4[:, 2:4]
        ft_3_ = out4[:, 4:]
