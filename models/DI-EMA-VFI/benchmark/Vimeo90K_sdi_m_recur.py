...
if __name__ == '__main__':
    ...
    CUDA_VISIBLE_DEVICES=4 screen python Vimeo90K_sdi_m_recur.py --testset_path /mnt/disks/ssd0/dataset/vimeo_septuplet/ --iters 2
    ...
    ************************************************************
    CUDA_VISIBLE_DEVICES=0 screen python Vimeo90K_sdi_m_recur.py --testset_path /mnt/disks/ssd0/dataset/vimeo_septuplet/ --unif --iters 1 --checkpoint ../experiments/EMA-VFI_m_recur/train_sdi_log/
    CUDA_VISIBLE_DEVICES=1 screen python Vimeo90K_sdi_m_recur.py --testset_path /mnt/disks/ssd0/dataset/vimeo_septuplet/ --unif --iters 2 --checkpoint ../experiments/EMA-VFI_m_recur/train_sdi_log/
    CUDA_VISIBLE_DEVICES=2 screen python Vimeo90K_sdi_m_recur.py --testset_path /mnt/disks/ssd0/dataset/vimeo_septuplet/ --unif --iters 3 --checkpoint ../experiments/EMA-VFI_m_recur/train_sdi_log/
    """
    parser = argparse.ArgumentParser()
    parser.add_argument('--checkpoint', type=str, default='../experiments/EMA-VFI_sdi_m_recur/train_sdi_log/',
    ...
    if args.unif:
        logger = Logger(osp.join(args.checkpoint, 'iters{}_unif_test_log.txt'.format(args.iters)))
    else:
        logger = Logger(osp.join(args.checkpoint, 'iters{}_test_log.txt'.format(args.iters)))
    ...
    for i in f:
        ...
        if (len(name) <= 1):
            continue
        # if path + 'sequences/' + name + '/im1.png' != '/mnt/disks/ssd0/dataset/vimeo_septuplet/sequences/00088/0794/im1.png':
        #     if not flag:
        #         continue
        # else:
        #     flag = True
        ...
        for embt, gt_index in zip(embts, gt_indices):
            ...
            for j in range(args.iters):
                ...
                mid = model.inference(I0, I1_tensor, pre_mid, cur_embt, pre_embt)
                pre_mid = mid
