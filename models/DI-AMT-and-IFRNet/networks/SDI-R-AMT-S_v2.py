...
from networks.blocks.ifrnet_recur_v2 import (
    resize,
    Encoder,
    InitDecoder,
    IntermediateDecoder
)
from networks.blocks.multi_flow_recur_v2 import (
    multi_flow_combine,
    MultiFlowDecoder
)
...
class Model(nn.Module):
    ...
    def _corr_scale_lookup(self, corr_fn, coord, flow0, flow1, embt, downsample=1):
        ...
        t1_scale = 1. / (embt + 1e-6)
        t0_scale = 1. / (1. - embt + 1e-6)
        if downsample != 1:
            inv = 1 / downsample
    ...
    def forward(self, img0, img1, img_ref, embt, embt_ref, scale_factor=1.0, eval=False, **kwargs):
        mean_ = torch.cat([img0, img1], 2).mean(1, keepdim=True).mean(2, keepdim=True).mean(3, keepdim=True)
        img0 = img0 - mean_
        img1 = img1 - mean_
        img_ref = img_ref - mean_
        img0_ = resize(img0, scale_factor) if scale_factor != 1.0 else img0
        img1_ = resize(img1, scale_factor) if scale_factor != 1.0 else img1
        img_ref_ = resize(img_ref, scale_factor) if scale_factor != 1.0 else img_ref
        b, _, h, w = img0_.shape
        ...
        f_ref_1, f_ref_2, f_ref_3, f_ref_4 = self.encoder(img_ref_)

        ######################################### the 4th decoder #########################################
        embt_4 = resize(embt, scale_factor=1. / 16)
        embt_3 = resize(embt, scale_factor=1. / 8)
        embt_2 = resize(embt, scale_factor=1. / 4)
        embt_1 = resize(embt, scale_factor=1. / 2)
        embt_ref_4 = resize(embt_ref, scale_factor=1. / 16)
        up_flow0_4, up_flow1_4, ft_3_ = self.decoder4(f0_4, f1_4, f_ref_4, embt_4, embt_ref_4)
        ...
        ######################################### the 3rd decoder #########################################
        up_flow0_3, up_flow1_3, ft_2_ = self.decoder3(ft_3_, f0_3, f1_3, up_flow0_4, up_flow1_4, embt_3)
        ...
        ######################################### the 2nd decoder #########################################
        up_flow0_2, up_flow1_2, ft_1_ = self.decoder2(ft_2_, f0_2, f1_2, up_flow0_3, up_flow1_3, embt_2)
        ...
        ######################################### the 1st decoder #########################################
        up_flow0_1, up_flow1_1, mask, img_res = self.decoder1(ft_1_, f0_1, f1_1, up_flow0_2, up_flow1_2, embt_1)
